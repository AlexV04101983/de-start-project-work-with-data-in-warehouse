...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    top_craftsman_for_customer VARCHAR NOT NULL, -- идентификатор самого популярного мастера ручной работы у заказчика.
...
dwh_delta AS ( -- определяем, какие данные были изменены в витрине или добавлены в DWH. Формируем дельту изменений
    SELECT     
            ...
            crd.customer_id AS exist_customer_id, --exist_craftsman_id
...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. Этой информации по клиентам в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
    SELECT  
            ...
            T4.popular_craftsman_id AS top_craftsman_for_customer,
                        ...
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у заказчика. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                /*** Для столбца top_craftsman_for_customer ***/
								INNER JOIN (
									SELECT DISTINCT ON (dd.customer_id)
										dd.customer_id AS customer_id_for_top_craft,
										dd.craftsman_id AS popular_craftsman_id,
										COUNT(dd.order_id) AS order_count
									FROM dwh_delta AS dd
									GROUP BY dd.customer_id, dd.craftsman_id
									ORDER BY dd.customer_id, order_count DESC
								) AS T5 ON T2.customer_id = T5.customer_id_for_top_craft
                ) AS T4 WHERE T4.rank_count_product = 1
